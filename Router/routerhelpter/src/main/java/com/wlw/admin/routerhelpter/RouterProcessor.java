...
import com.wlw.admin.annotation.Component;
...
public class RouterProcessor extends AbstractProcessor {
    ...
    private void processInstaller(Set<? extends Element> mainAppElement) throws IOException {
        ...
        writer.print("public class " + Config.ROUTER_MANAGER + " {");
    ...
    private void processComponent(RoundEnvironment roundEnvironment) throws IOException {
    ...
    private void writeComponentFile(String componentName) throws IOException {
        ...
        for (Map.Entry<String, String> entry : mStaticRouterMap.entrySet()) {
            printWriter.println("com.wlw.admin.routers.Router.router(\"" + entry.getKey() + "\"," + entry.getValue() + ".class);");
        }


        for (String klass : mAutoRouterList) {
            printWriter.println("if (Activity.class.isAssignableFrom(" + klass + ".class)){");
            printWriter.println("com.wlw.admin.routers.Router.router(com.wlw.admin.routers.rule.ActivityRule.ACTIVITY_SCHEME+\"" +
                    klass + "\"," + klass + ".class);");
            printWriter.println("}");


            printWriter.println("else if(Service.class.isAssignableFrom(" + klass + ".class)){");
            printWriter.println("com.wlw.admin.routers.Router.router(com.wlw.admin.routers.rule.ServiceRule.SERVICE_SCHEME+\"" +
                    klass + "\"," + klass + ".class);");
            printWriter.println("}");

            printWriter.println("else if(BroadcastReceiver.class.isAssignableFrom(" + klass + ".class)){");
            printWriter.println("com.wlw.admin.routers.Router.router(com.wlw.admin.routers.rule.ReceiverRule.RECEIVER_SCHEME+\"" +
                    klass + "\"," + klass + ".class);");
            printWriter.println("}");
        }
        printWriter.println("}");
